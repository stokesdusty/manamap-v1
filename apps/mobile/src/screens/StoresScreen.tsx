...
import { colors, radii, shadows, spacing, typography, type ManaColor } from '../theme';
...
const STORE_ACCENT_PALETTE: ManaColor[] = ['W', 'U', 'B', 'R', 'G'];

function storeAccentColor(storeId?: string | null): string {
  if (!storeId) return colors.accent;
  let hash = 0;
  for (let i = 0; i < storeId.length; i++) {
    hash = (hash * 31 + storeId.charCodeAt(i)) >>> 0;
  }
  return colors.mana[STORE_ACCENT_PALETTE[hash % STORE_ACCENT_PALETTE.length]];
}
...
function AttendeeRow({ entry }: { entry: EventAttendeeEntry }) {
  const fill = entry.avatarColors[0]
    ? ({ W: colors.mana.W, U: colors.mana.U, B: colors.mana.B, R: colors.mana.R, G: colors.mana.G }[
        entry.avatarColors[0]
      ] ?? colors.border)
    : colors.border;
  const textFill = ['W', 'G'].includes(entry.avatarColors[0])
    ? colors.textPrimary
    : colors.textInverse;
  return (
    <View style={attendeeRow.root}>
      ...
      <Text style={attendeeRow.name} numberOfLines={1}>
        {entry.displayName}
      </Text>
      {entry.isHereNow && (
...
function EventRow({ event, storeId }: { event: StoreEvent; storeId: string }) {
  ...
  const { data: attendance } = useEventAttendance(
    expanded ? storeId : null,
      ...
      <View style={evtRow.metaRow}>
        {event.formatName && (
          <View
            style={[
              evtRow.chip,
              { borderColor: sourceColor + '60', backgroundColor: sourceColor + '12' },
            ]}
          >
            <Text style={[evtRow.chipText, { color: sourceColor }]}>{event.formatName}</Text>
          </View>
        )}
        {event.attendeeCount > 0 && <Text style={evtRow.going}>{event.attendeeCount} going</Text>}
...
function HereNowRow({ player }: { player: NearbyPlayer }) {
  ...
  return (
    <View style={hereRow.root}>
      ...
      <Text style={hereRow.name} numberOfLines={1}>
        {player.displayName}
      </Text>
      {player.metBefore && (
...
function StoreDetailSheet({
  ...
  const accent = storeAccentColor(store?.id);
  const { data: eventDays = [], isLoading: eventsLoading } = useStoreEvents(
    proposed ? null : storeId,
  );
  ...
  const { data: redemptionStatus } = useRedemptionStatus(
    claimResult?.offerId ?? null,
    claimResult !== null,
  );
  ...
  const [tooFarError, setTooFarError] = useState<{
        ...
        if (axios.isAxiosError(err) && err.response?.status === 422) {
          ...
          if (
            ...
            setTooFarError({
              ...
              allowedMeters: body.allowedMeters,
            });
          }
        }
      },
  ...
  return (
    <>
                  ...
                  {offers.map((offer) => (
                    <View key={offer.id} style={sheet.offerChip}>
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        {offer.description ? (
                          <Text style={sheet.offerChipDesc} numberOfLines={2}>
                            {offer.description}
                          </Text>
                        ) : null}
                        {offer.type === 'STREAK' && offer.streakRequired ? (
                          <Text style={sheet.offerChipHint}>
                            Requires {offer.streakRequired}-week streak
                          </Text>
                        ) : offer.type === 'FIRST_VISIT' ? (
                          <Text style={sheet.offerChipHint}>
                            First visit bonus — check in to unlock
                          </Text>
                        ) : null}
                      </View>
                    </View>
                  ...
                  <>
                    {leaderboard?.entries.map((entry) => (
                      <View key={entry.userId} style={[sheet.lbRow, entry.isMe && sheet.lbRowMe]}>
                        ...
                        <View style={sheet.lbAvatar}>
                          <Text style={sheet.lbAvatarText}>
                            {entry.displayName.charAt(0).toUpperCase()}
                          </Text>
                        </View>
                        <Text style={sheet.lbName} numberOfLines={1}>
                          {entry.displayName}
                        </Text>
                    ...
                    {leaderboard?.myEntry && !leaderboard.entries.some((e) => e.isMe) && (
                      <View style={[sheet.lbRow, sheet.lbRowMe]}>
                        ...
                        <View style={sheet.lbAvatar}>
                          <Text style={sheet.lbAvatarText}>Y</Text>
                        </View>
                        <Text style={sheet.lbName}>You</Text>
                        <View style={sheet.lbStats}>
                          <Text style={sheet.lbStreak}>
                            🔥 {leaderboard.myEntry.currentStreak}w
                          </Text>
                          <Text style={sheet.lbTotal}>
                            {leaderboard.myEntry.totalCheckins} visits
                          </Text>
                        </View>
                      </View>
                    )}
              ...
              {!proposed && tooFarError && (
                <View style={sheet.tooFarBox}>
                  ...
                  <Text style={sheet.tooFarBody}>
                    You're ~{tooFarError.distanceMeters}m from {store?.name ?? 'this store'} — get
                    within {tooFarError.allowedMeters}m to check in.
                  </Text>
                  <Pressable
      ...
      <BadgeEarnedSheet badges={earnedBadges} onDismiss={() => setEarnedBadges([])} />
            ...
            <View style={promoStyles.card}>
              ...
              {activeEvents.map((evt) => (
                <Pressable
                  key={evt.id}
                  style={({ pressed }) => [
                    eventTagStyles.eventBtn,
                    (pressed || isAssociating) && { opacity: 0.7 },
                  ]}
                ...
                >
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={eventTagStyles.eventTime}>
                      {formatTime(evt.startsAt)}
                      {evt.formatName ? ` · ${evt.formatName}` : ''}
                    </Text>
                  </View>
        ...
        >
          <Pressable style={promoStyles.backdrop} onPress={() => setEligibleOffers([])}>
            <View style={promoStyles.card}>
              ...
              <Text style={[promoStyles.heading, { color: accent }]}>You unlocked a reward!</Text>
              {eligibleOffers.map((offer) => (
                <View key={offer.id} style={promoStyles.offerBlock}>
                  ...
                  {offer.terms ? <Text style={promoStyles.terms}>{offer.terms}</Text> : null}
                  <Pressable
                    style={({ pressed }) => [
                      claimStyles.redeemBtn,
                      { backgroundColor: accent },
                      (pressed || isClaiming) && { opacity: 0.7 },
                    ]}
                    disabled={isClaiming}
                    onPress={() => {
                      claimOffer(
          ...
          <Pressable style={promoStyles.backdrop} onPress={() => setClaimResult(null)}>
            <View style={[promoStyles.card, { alignItems: 'center' }]}>
              ...
              {effectiveStatus === 'REDEEMED' ? (
                ...
                <>
                  <Text style={[claimStyles.codeDisplay, { color: accent }]}>
                    {claimResult.code}
                  </Text>
                  <View style={[claimStyles.qrWrapper, { borderColor: accent }]}>
              ...
              <Pressable
                style={({ pressed }) => [
                  promoStyles.dismissBtn,
                  { backgroundColor: accent, marginTop: 16 },
                  pressed && { opacity: 0.8 },
                ]}
...
export function StoresScreen({ navigation, route }: StoresScreenProps) {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['bottom', 'left', 'right']}>
      ...
      <StoreSuggestSheet visible={showSuggest} onClose={() => setShowSuggest(false)} />
    </SafeAreaView>
  );
}
